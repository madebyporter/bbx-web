<template>
  <div class="flex flex-col h-full">
    ...
    <div v-else-if="!resource" class="flex flex-col items-center justify-center p-8 h-full grow text-neutral-500">
      ...
      <p class="text-sm">The kit you're looking for doesn't exist.</p>
      <NuxtLink to="/kits" class="mt-4 text-amber-400 hover:text-amber-500 transition-colors">
        Browse all kits
      </NuxtLink>
    </div>
...
definePageMeta({
  key: (route) => `kits-${route.params.slug}`
})
...
const { data: resourceData, pending: loading, refresh } = await useAsyncData(
  () => `resource-kit-${route.params.slug}`,
  async () => {
    ...
    try {
      const data = await fetchResourceBySlug(slug, 'sounds')
      return data
    } catch (error) {
