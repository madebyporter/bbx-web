<template>
  <div class="flex flex-col h-full">
    ...
    <div v-else-if="!resource" class="flex flex-col items-center justify-center p-8 h-full grow text-neutral-500">
      ...
      <p class="text-sm">The software you're looking for doesn't exist.</p>
      <NuxtLink to="/software" class="mt-4 text-amber-400 hover:text-amber-500 transition-colors">
        Browse all software
      </NuxtLink>
    </div>
        ...
        <div class="p-4 border-b border-neutral-800">
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col md:flex-row gap-4">
              <div class="w-full md:w-64 h-48 md:h-64 bg-neutral-200 rounded-md overflow-hidden shrink-0">
                <img
              ...
              <div class="flex flex-col gap-2 flex-1">
                <h1 class="text-2xl md:text-3xl font-bold">{{ resource.name }}</h1>
                <div class="flex flex-col gap-1 text-sm text-neutral-400">
                  <div><span class="text-neutral-500">Creator:</span> {{ resource.creator }}</div>
                  <div><span class="text-neutral-500">Price:</span> {{ resource.price }}</div>
                  <div class="flex flex-row gap-2 items-center">
                    <span class="text-neutral-500">OS:</span>
            ...
            <!-- Tags -->
            <div v-if="resource.tags && resource.tags.length > 0" class="flex flex-row gap-2 items-center flex-wrap">
...
// Define page meta with key to force re-render on route change
definePageMeta({
...
// Debug logging - this should run when component is created
if (process.client) {
  console.log('[software/[slug].vue] Component created on client', {
    path: route.path,
    params: route.params,
...
if (process.server) {
  console.log('[software/[slug].vue] Component created on server', {
    path: route.path,
    params: route.params,
    slug: route.params.slug
  })
...
// Use route.path as key to ensure uniqueness on route changes
const { data: resourceData, pending: loading, refresh } = await useAsyncData(
  () => `resource-software-${route.params.slug}`,
  async () => {
    console.log('[software/[slug].vue] Fetching resource data', {
      slug: route.params.slug,
      path: route.path
    })
    
    if (!supabase) {
      console.log('[software/[slug].vue] No supabase client')
      return null
    }
    ...
    try {
      const data = await fetchResourceBySlug(slug, 'software')
      console.log('[software/[slug].vue] Resource data fetched', {
        found: !!data,
        name: data?.name
      })
      return data
    } catch (error) {
      console.error('[software/[slug].vue] Error fetching resource:', error)
      return null
    }
  },
  {
...
watch(() => route.path, (newPath, oldPath) => {
  console.log('[software/[slug].vue] Route path changed', {
    from: oldPath,
    to: newPath,
    slug: route.params.slug
  })
...
watch(() => route.params.slug, async (newSlug, oldSlug) => {
  console.log('[software/[slug].vue] Slug param changed', {
    from: oldSlug,
    to: newSlug
  })
...
watch(() => resource.value, (newResource) => {
  ...
  const seoTitle = `${newResource.name} - Music Production Software`
  const seoDescription = `${newResource.name} by ${newResource.creator}. ${newResource.price}. ${newResource.tags?.join(', ')}.`
  const seoUrl = `${siteUrl}/software/${route.params.slug}`
...
// Lifecycle hook to verify component is mounting
onMounted(() => {
  console.log('[software/[slug].vue] Component mounted!', {
    path: route.path,
    params: route.params,
    slug: route.params.slug,
